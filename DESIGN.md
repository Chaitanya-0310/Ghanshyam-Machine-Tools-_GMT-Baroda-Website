...
- Premium refinement: the exploded three-jaw chuck receives a dedicated full-width product stage immediately below the opening copy. The stage is framed as a quiet studio object with no labels, technical annotations, calibration marks, or glass panels over the imagery.
- Hero product object: the product stage uses the supplied 300-frame PNG assembly sequence from `ezgif-7062f628291e578f-png-split.zip`. It is presented as a visual product object, not technical CAD or a specification claim. Mobile and reduced-motion visitors receive the assembled final frame.
...
- Homepage signature: the supplied lathe chuck assembles inside a dedicated native-ratio studio frame below the hero copy. The uninterrupted object stage, rather than overlaid interface decoration, carries the visual identity.
...
- Hero assembly sequence: desktop visitors enter a dedicated full-width 16:9 product study immediately after the text-led hero. The supplied 300-frame exploded three-jaw-chuck sequence holds briefly on the exploded layout, scrubs continuously through every frame, then holds the assembled result before release. Framing remains centred and uncropped throughout; no late zoom, focal shift, phase labels, calibration graphics, or progress card sit over the object. The stage is capped near the source's native 1280×720 size, uses a bounded frame-loading window, and draws at an appropriate device-pixel ratio. Reduced-motion, tablet, and mobile visitors receive the final static assembly frame.
